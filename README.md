# taobao_taojinbi
本项目主要用于自动执行淘金币相关任务，参与淘金币夺宝活动(可下注抢茅台)
...
**"年货节浇灌福气"按钮在主界面下拉可见，关于升级只加0.02%的问题，有如下解决方法：**
1. 使用淘宝执行任务后关闭autojs程序，清除淘宝的缓冲和数据(手机设置->应用->淘宝)，重新登录浇灌升级，本人实测可行 (可累积多了再升级)

**支付宝领福卡方法**:
1. 在autojs里面导入fucard.js，编辑该脚本在第一行填入自己的手机号码
2. 运行运行脚本即可 (目前只在华为机上测试通过)

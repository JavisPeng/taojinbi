...
//===================用户可编辑参数===================
var MAX_ALL_TASK_EPOCH = 3 //所有任务重复次数,解决新增任务问题
var MAX_EPOCH = 101 //浏览任务最大执行次数
var wait_sec = 15 //任务执行默认等待的时长 考虑到网络卡顿问题 默认15秒
var do_notification_task = 1 //1表示自动执行[开启淘宝通知权限任务],0表示跳过不执行
...
//逛好店并领10金币
function shop_10coin_task(not_key_reg_str, btn_reg_str) {
...
function multi_choice() {
    do_dice_task = do_baba_farm_task = do_xiaoxiaole_task = num_ant_find = do_tianmao_task = do_notification_task = is_earn_10coin = 0
    let options = dialogs.multiChoice("(作者:Javis486)请选择需要执行的任务", ['淘宝人生掷色子任务', '逛农场领免费水果任务', '消消乐任务', '蚂蚁森林任务', '天猫APP领红包任务', '开启淘宝通知权限任务', '逛好店并领10金币任务', '全部任务'])
    options.forEach(option => {
        switch (option) {
            ...
            case 6:
                is_earn_10coin = 1; break;
            case 7:
                do_dice_task = do_baba_farm_task = do_xiaoxiaole_task = num_ant_find = do_tianmao_task = do_notification_task = is_earn_10coin = 1; break
        }
    });
}
